...
public class ComputeModuleF90RatioCommentStatistics implements MeasureComputer {
	...
	public MeasureComputerDefinition define(MeasureComputerDefinitionContext defContext) {
	    ...
	    String[] metricTab = new String[] {ICodeMetricsF90RatioComment.F90_RATIO_COMMENT.key(), ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MEAN.key(), ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MIN.key(), ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MAX.key()};
	...
	public void compute(MeasureComputerContext context) {
		...
		if (context.getComponent().getType() != Component.Type.FILE) {
			...
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT.key());
			compute(context, childrenMeasures);
			
			// Search RATIO_COMMENT mean measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MEAN.key());
			computeMean(context, childrenMeasures);

			// Search RATIO_COMMENT minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MIN.key());
			computeMin(context, childrenMeasures);
						
			// Search RATIO_COMMENT minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MAX.key());
	...
	private void computeMax(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MAX.key(), max);
		}
	}
	...
	private void computeMin(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MIN.key(), min);
		}
	}
	...
	private void computeMean(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT_MEAN.key(),(nbItem!=0)?sum/nbItem:sum);
		}
	}
	...
	private void compute(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90RatioComment.F90_RATIO_COMMENT.key(),sum);
		}
	}
}