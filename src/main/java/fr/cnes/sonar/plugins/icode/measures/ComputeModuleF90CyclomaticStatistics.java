...
public class ComputeModuleF90CyclomaticStatistics implements MeasureComputer {
	...
	public MeasureComputerDefinition define(MeasureComputerDefinitionContext defContext) {
	    ...
	    String[] metricTab = new String[] {ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC.key(), ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MEAN.key(), ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MIN.key(), ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MAX.key()};
	...
	public void compute(MeasureComputerContext context) {
		...
		if (context.getComponent().getType() != Component.Type.FILE) {
			...
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MEAN.key());
			computeMean(context, childrenMeasures);

			// Search Cyclomatic minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MIN.key());
			computeMin(context, childrenMeasures);
						
			// Search Cyclomatic minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MAX.key());
	...
	private void computeMax(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MAX.key(), max);
		}
	}
	...
	private void computeMin(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MIN.key(), min);
		}
	}
	...
	private void computeMean(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Cyclomatic.F90_CYCLOMATIC_MEAN.key(),(nbItem!=0)?sum/nbItem:sum);
		}
	}
}