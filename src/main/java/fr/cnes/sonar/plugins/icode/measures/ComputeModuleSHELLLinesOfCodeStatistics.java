...
public class ComputeModuleSHELLLinesOfCodeStatistics implements MeasureComputer {
	...
	public MeasureComputerDefinition define(MeasureComputerDefinitionContext defContext) {
	    ...
	    String[] metricTab = new String[] {ICodeMetricsSHELLLinesOfCode.SHELL_LOC.key(), ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MEAN.key(), ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MIN.key(), ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MAX.key()};
	...
	public void compute(MeasureComputerContext context) {
		...
		if (context.getComponent().getType() != Component.Type.FILE) {
			...
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC.key());
			compute(context, childrenMeasures);
			
			// Search Lines of Code mean measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MEAN.key());
			computeMean(context, childrenMeasures);

			// Search Lines of Code minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MIN.key());
			computeMin(context, childrenMeasures);
						
			// Search Lines of Code minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MAX.key());
	...
	private void computeMax(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MAX.key(), max);
		}
	}
	...
	private void computeMin(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MIN.key(), min);
		}
	}
	...
	private void computeMean(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MEAN.key(),(nbItem!=0)?sum/nbItem:sum);
		}
	}
	...
	private void compute(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC.key(),sum);
		}
	}
}