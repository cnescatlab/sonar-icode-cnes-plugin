...
public class ComputeModuleF90NestingStatistics implements MeasureComputer {
	...
	public MeasureComputerDefinition define(MeasureComputerDefinitionContext defContext) {
	    ...
	    String[] metricTab = new String[] {ICodeMetricsF90Nesting.F90_NESTING.key(), ICodeMetricsF90Nesting.F90_NESTING_MEAN.key(), ICodeMetricsF90Nesting.F90_NESTING_MIN.key(), ICodeMetricsF90Nesting.F90_NESTING_MAX.key()};
	...
	public void compute(MeasureComputerContext context) {
		...
		if (context.getComponent().getType() != Component.Type.FILE) {
			...
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING.key());
			compute(context, childrenMeasures);
			
			// Search NESTING mean measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MEAN.key());
			computeMean(context, childrenMeasures);

			// Search NESTING minimum measure for children files
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MIN.key());
			computeMin(context, childrenMeasures);
						
			childrenMeasures = context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MAX.key());
	...
	private void computeMax(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MAX.key(), max);
		}
	}
	...
	private void computeMin(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MIN.key(), min);
		}
	}
	...
	private void computeMean(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MEAN.key(),(nbItem!=0)?sum/nbItem:sum);
		}
	}
	...
	private void compute(MeasureComputerContext context, Iterable<Measure> childrenMeasures) {
		if(childrenMeasures.iterator().hasNext()){
			...
			context.addMeasure(ICodeMetricsF90Nesting.F90_NESTING.key(),sum);
		}
	}
}