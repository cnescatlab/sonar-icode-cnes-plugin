...
public class ComputeModuleSHELLNestingStatisticsTest {
	...
	public void callDefine() {
		...
		List<Metric> allMetrics = (new ICodeMetricsSHELLNesting()).getMetrics();
		...
		ComputeModuleSHELLNestingStatistics compute = new ComputeModuleSHELLNestingStatistics();
	...
	public void given_context_when_compute_then_newComputedMeasures(){
		...
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLNesting.SHELL_NESTING.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLNesting.SHELL_NESTING_MEAN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLNesting.SHELL_NESTING_MIN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLNesting.SHELL_NESTING_MAX.key())).thenReturn(childsMeasures);
		ComputeModuleSHELLNestingStatistics computeModule = new ComputeModuleSHELLNestingStatistics();
		computeModule.compute(context);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLNesting.SHELL_NESTING.key(), 6);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLNesting.SHELL_NESTING_MEAN.key(), 3.0);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLNesting.SHELL_NESTING_MIN.key(), 1);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLNesting.SHELL_NESTING_MAX.key(), 5);		
	}
