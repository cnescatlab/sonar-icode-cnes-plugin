...
public class ComputeModuleSHELLLinesOfCodeStatisticsTest {
	...
	public void callDefine() {
		...
		List<Metric> allMetrics = (new ICodeMetricsSHELLLinesOfCode()).getMetrics();
		...
		ComputeModuleSHELLLinesOfCodeStatistics compute = new ComputeModuleSHELLLinesOfCodeStatistics();
	...
	public void given_context_when_compute_then_newComputedMeasures(){
		...
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MEAN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MIN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MAX.key())).thenReturn(childsMeasures);
		ComputeModuleSHELLLinesOfCodeStatistics computeModule = new ComputeModuleSHELLLinesOfCodeStatistics();
		computeModule.compute(context);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC.key(), 6);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MEAN.key(), 3.0);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MIN.key(), 1);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLLinesOfCode.SHELL_LOC_MAX.key(), 5);		
	}
