...
public class ComputeModuleSHELLCyclomaticStatisticsTest {
	...
	public void callDefine() {
		...
		List<Metric> allMetrics = (new ICodeMetricsSHELLCyclomatic()).getMetrics();
		...
		ComputeModuleSHELLCyclomaticStatistics compute = new ComputeModuleSHELLCyclomaticStatistics();
	...
	public void given_context_when_compute_then_newComputedMeasures(){
		...
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MEAN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MIN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MAX.key())).thenReturn(childsMeasures);
		ComputeModuleSHELLCyclomaticStatistics computeModule = new ComputeModuleSHELLCyclomaticStatistics();
		computeModule.compute(context);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MEAN.key(), 3.0);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MIN.key(), 1);
		Mockito.verify(context).addMeasure(ICodeMetricsSHELLCyclomatic.SHELL_CYCLOMATIC_MAX.key(), 5);		
	}
}
