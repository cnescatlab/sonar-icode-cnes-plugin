...
public class ComputeModuleF90NestingStatisticsTest {
	...
	public void callDefine() {
		...
		List<Metric> allMetrics = (new ICodeMetricsF90Nesting()).getMetrics();
		...
		ComputeModuleF90NestingStatistics compute = new ComputeModuleF90NestingStatistics();
	...
	public void given_context_when_compute_then_newComputedMeasures(){
		...
		Mockito.when(context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MEAN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MIN.key())).thenReturn(childsMeasures);
		Mockito.when(context.getChildrenMeasures(ICodeMetricsF90Nesting.F90_NESTING_MAX.key())).thenReturn(childsMeasures);
		ComputeModuleF90NestingStatistics computeModule = new ComputeModuleF90NestingStatistics();
		computeModule.compute(context);
		Mockito.verify(context).addMeasure(ICodeMetricsF90Nesting.F90_NESTING.key(), 6);
		Mockito.verify(context).addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MEAN.key(), 3.0);
		Mockito.verify(context).addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MIN.key(), 1);
		Mockito.verify(context).addMeasure(ICodeMetricsF90Nesting.F90_NESTING_MAX.key(), 5);		
	}
